...
exports.require_tree = function(uPath, options) {
  var appendPackage, extend, fs, getPackage, getPwd, initial, packages, parsePath, path, walker, _root,
    _this = this;
          ...
          try {
            if (!options.preserve_filenames) {
              ...
              if (name.match(/^index+/)) {
                ...
                o = (o = getPackage(initial(pwd.split(path.sep)).join(path.sep))) != null ? o : appendPackage((parsePath(pwd)).join(path.sep));
  ...
  packages.require_tree.getPackage = exports.getPackage = function(p) {
