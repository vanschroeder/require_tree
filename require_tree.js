...
exports.require_tree = function(uPath, options) {
  ...
  walker = (function(_this) {
    return function(dir) {
      var d, e, file, list, m, n, name, o, p, pwd, r, stat, v, x, _i, _len;
            ...
            try {
              if (!options.preserve_filenames) {
                if (typeof (x = require(fs.realpathSync("" + file))) !== 'function' && !(x instanceof Array)) {
                  ...
                  (m = {})[(n = name.split('.').shift()) === 'index' ? ((d = dirname(pwd).split(path.sep).pop()) === '' ? _root.split(path.sep).pop() : d) : n] = x;
